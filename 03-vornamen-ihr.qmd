...
1. Passt die Visualisierung mittels des statR R-Pakets an die Corporate Identity des Kantons Zürich an.
...
ggplot(data = vornamen_xy, aes(x = geburtsjahr, 
    ...
    geom_line() +
    facet_wrap(~summe) +
    scale_color_manual(values = zhpal$zhwebaccent) +
    theme_stat()
```
...
1. Schreibt eine Fragen auf und versucht sie mittels R-Code zu beantworten.
